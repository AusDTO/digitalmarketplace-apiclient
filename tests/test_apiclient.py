...
class TestDataApiClient(object):
    ...
    def test_get_domains_generic_with_data_and_params(self, data_client, rmock):
        rmock.get(
            "http://baseurl/domains",
            json={"status": "ok"},
            status_code=200)

        result = data_client.req.domains().get(data={}, params={})
    ...
    def test_can_update_terms_acceptance_timestamp(self, data_client, rmock):
        rmock.post(
            'http://baseurl/users/123',
            json={},
            status_code=200)
        data_client.update_user(123, updater='test@example.com', fields={'termsAcceptedAt': 'time'})
        assert rmock.called
        assert rmock.last_request.json() == {
            'updated_by': 'test@example.com',
            'users': {'termsAcceptedAt': 'time'}
        }
    ...
    def test_find_suppliers_with_per_page_parameter(self, data_client, rmock):
        rmock.get(
            "http://baseurl/suppliers?per_page=1000",
            json={"suppliers": "result"},
            status_code=200)

        result = data_client.find_suppliers(per_page=1000)
